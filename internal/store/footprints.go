...
func (s *Store) Footprints() ([]models.Footprint, error) {
	rows, err := s.db.Query(`
		SELECT id, country_code, country_name, province, city, note, sort_order
	...
	for rows.Next() {
		...
		if err := rows.Scan(&f.ID, &f.CountryCode, &f.CountryName, &f.Province, &f.City, &f.Note, &f.SortOrder); err != nil {
			return nil, err
		}
		out = append(out, f)
	}
...
func (s *Store) Footprint(id int64) (models.Footprint, error) {
	...
	err := s.db.QueryRow(`
		...
		Scan(&f.ID, &f.CountryCode, &f.CountryName, &f.Province, &f.City, &f.Note, &f.SortOrder)
	return f, err
}
...
func (s *Store) CreateFootprint(f models.Footprint) (int64, error) {
	res, err := s.db.Exec(`
		INSERT INTO footprints (country_code, country_name, province, city, note, sort_order)
		VALUES (?, ?, ?, ?, ?, ?)`,
		f.CountryCode, f.CountryName, f.Province, f.City, f.Note, f.SortOrder)
	if err != nil {
		return 0, err
	}
...
func (s *Store) UpdateFootprint(f models.Footprint) error {
	_, err := s.db.Exec(`
		UPDATE footprints SET country_code = ?, country_name = ?, province = ?, city = ?, note = ?, sort_order = ?
		WHERE id = ?`,
		f.CountryCode, f.CountryName, f.Province, f.City, f.Note, f.SortOrder, f.ID)
	return err
}
